...
```
esi - уникальное значение для каждого lag во всей фабрике.\
system-id - **На сколько я понял, это значение может быть одинаковым на коробке в разных esi, хотелось бы получить коментарий тут.**
